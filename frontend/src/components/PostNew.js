...
const PostNew = () => {
  ...
  const fetchPosts = async () => {
    try {
      ...
      console.log(
        "Post statuses:",
        postsData.map((post) => post.status)
      );
      ...
      const formattedArticles = recentPosts.map((post) => ({
        ...
        title: post.title,
        image: post.image
          ? `http://localhost:8080/uploads/posts/${post.image}`
  ...
  return (
    <div className="container mx-auto px-4 md:px-8 lg:px-16">
      <div className="flex justify-between items-center mt-20 mb-4">
        <h2 className="text-2xl md:text-3xl font-mono text-gray-800" style={{ fontFamily: 'Dancing Script' }}>
          BÀI VIẾT MỚI NHẤT
        </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6">
        {articles.length > 0 && (
          <div className="col-span-1 md:col-span-2 flex flex-col">
            <Link
              to={articles[0].link}
              className="block overflow-hidden border border-gray-300 shadow-md hover:shadow-xl transition-shadow h-full"
            >
              ...
              <div className="p-4 md:p-5 flex flex-col flex-grow bg-white">
                <h3 className="text-lg font-mono mb-2 text-gray-800">
                  {articles[0].title}
                </h3>
            ...
            {articles.slice(1).map((article, index) => (
              ...
              >
                ...
                <div className="p-2 md:p-3 flex flex-col justify-center flex-grow bg-white">
                  <h3 className="text-sm font-mono mb-1 text-gray-800 line-clamp-2">
                    {article.title}
                  </h3>

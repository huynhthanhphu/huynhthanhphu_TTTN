...
useEffect(() => {
    axios.get(`http://localhost:8080/api/products/${id}`)
        .then((res) => {
            ...
            const subImagesRaw = res.data.images?.map(img => img.imageUrl) || [];
            
            // Làm sạch ảnh phụ: tránh thêm ảnh chính, tránh URL đã đầy đủ
            const cleanedSubImages = subImagesRaw
                .map(img => {
                    // Nếu là URL đầy đủ, chỉ lấy tên file
                    if (img.startsWith('http')) {
                        return img.split('/').pop(); // Lấy tên ảnh thôi
                    }
                    return img;
                })
                .filter(img => img && img !== mainImage); // tránh trùng ảnh chính và null/undefined
            
            // Tạo Set để loại bỏ duplicate, sau đó convert về array
            const uniqueSubImages = [...new Set(cleanedSubImages)];
            const allImages = [mainImage, ...uniqueSubImages];
            
            setProduct(res.data);
            setProductImages(allImages);
            setCurrentImage(0);
            console.log('Raw sub images:', subImagesRaw);
            console.log('Cleaned sub images:', cleanedSubImages);
                   ...
                   {productImages.length > 1 && (
                    <div className="flex space-x-2 mt-2">
                        {productImages.map((img, index) => (
                            <div
                                key={index}
                                className={`cursor-pointer border-2 ${currentImage === index ? 'border-[#EB1C24]' : 'border-transparent'}`}
                                onClick={() => setCurrentImage(index)} // chỉ đổi chỉ số, không đổi mảng ảnh
                            >
                                <img src={getImageUrl(img)} alt={`Thumbnail ${index}`} className="w-20 h-20 object-cover" />
                            </div>
                        ))}
                    </div>
                )}

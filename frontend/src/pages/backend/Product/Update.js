...
const ProductUpdate = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const brandsResponse = await axios.get('http://localhost:8080/api/brands');
                ...
                if (product) {
                    setForm({
                        ...
                        categoryId: product.categoryId || (categoriesResponse.data.length > 0 ? categoriesResponse.data[0].id : ''),
                        brandId: product.brandId || (brandsResponse.data.length > 0 ? brandsResponse.data[0].id : ''),
                    ...
                    // If product has an image, set the current image
                    if (product.image) {
                        setCurrentImage(`http://localhost:8080/uploads/products/${product.image}`);
                    }
                } else {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setError('Không thể cập nhật sản phẩm. Vui lòng kiểm tra lại thông tin và thử lại.');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="container mx-auto mt-8 px-4 font-mono">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-xl font-mono font-medium text-gray-800">
                    <span className="border-b-2 border-indigo-500 pb-1">Cập nhật sản phẩm</span>
                </h1>
                <button

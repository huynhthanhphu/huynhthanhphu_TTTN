...
const UpdateTopic = () => {
  ...
  // Fetch topic data when component is mounted
  useEffect(() => {
    const fetchTopic = async () => {
      try {
        ...
        if (data && data.data) {
          if (data.data.data) {
            topicData = data.data.data;
          } else if (data.data.topic) {
            topicData = data.data.topic;
          } else {
            topicData = data.data;
          }
        } else {
        ...
        if (topicData) {
          ...
          setStatus(
            topicData.status === 1 ||
            topicData.status === true ||
            topicData.status === 'true'
          );
        }
      } catch (err) {
        console.error('Error fetching topic details:', err);
        setError('Không thể lấy thông tin topic. Vui lòng thử lại sau.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await TopicService.update(id, topicData);
      alert('Cập nhật topic thành công!');
      navigate('/admin/topic');
    } catch (err) {
      console.error('Error updating topic:', err);
      setError('Có lỗi xảy ra khi cập nhật topic. Vui lòng thử lại.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="container mx-auto mt-8 px-4 font-mono">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-medium text-gray-700">
          <span className="border-b-2 border-blue-500 pb-1">Cập nhật Topic</span>
        </h1>
        <button
